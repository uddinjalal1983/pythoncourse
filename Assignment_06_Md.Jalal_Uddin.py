...
def exercise01():
	...
	df = pd.DataFrame(np.random.randint(0,100,size=(3,4)), columns=list('1234'))
...
def exercise09():
	...
	df = df.drop(df.index[len(df)-2 : len(df)])
	import plotly.plotly as py
	import plotly.graph_objs as go
	import plotly
	data = [go.Scatter(x=df.index, y=df.bpi)]
	#plotly.tools.set_credentials_file(username='SaquibAlam', api_key='Mv3tqKUxnmzt1KxamOQD')
	plotly_url = ''#py.plot(data)
...
def exercise11(n):
	...
	array_1d = np.array(list(range(n)))
...
def exercise12(n):
	...
	checkerboard_matrix = []
	for i in range(2*n):
		checkerboard_matrix.append([])
		for j in range(2*n):
			checkerboard_matrix[i].append(k)
			if k == 1:
				k = 0		
			else:
				k = 1
	checkerboard_matrix = np.array(checkerboard_matrix)
...
def exercise13(n):
	...
	s = pd.Series(np.random.randint(n,size=n),index=pd.date_range('1/1/2010', periods=n, freq='D'))
	c = s.cumsum()
	c.plot(kind='bar')
	#plt.show()
	# ------ Place code above here /\ /\ /\ ------ 
...
def exercise15():
	...
	df = exercise08()[0][['street','zip']].iloc[::5, :]
